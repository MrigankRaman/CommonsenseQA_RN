        ...
        for target in acs:
            ...
            try:
                ...
                if (source, target) not in adj_cpts and (target, source) not in adj_cpts and source!=target:
                    adj_cpts.append((source, target, wt['weight']))
                    adj_cpts.append((target, source, wt['weight']+17))
            except KeyError:
                if (source, target) not in non_adj_cpts and (target, source) not in non_adj_cpts and source!=target:
                    non_adj_cpts.append((source, target))
                    non_adj_cpts.append((target, source))
...
def main():
    get_non_adj_cpts('./data/csqa/grounded/train.grounded.jsonl', './data/cpnet/conceptnet.en.pruned.graph', 'cpt_pairs_1hop_train_reverse_hybrid.jsonl')
    get_non_adj_cpts('./data/csqa/grounded/dev.grounded.jsonl', './data/cpnet/conceptnet.en.pruned.graph', 'cpt_pairs_1hop_dev_reverse_hybrid.jsonl')
    #get_non_adj_cpts('./data/csqa/grounded/test.grounded.jsonl', './data/cpnet/conceptnet.en.pruned.graph', 'cpt_pairs_1hop_test_reverse_hybrid.jsonl')
