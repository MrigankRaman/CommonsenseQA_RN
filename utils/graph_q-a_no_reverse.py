...
def concepts2adj(node_ids, qmask, amask):
    global id2relation
    cids = np.array(node_ids, dtype=np.int32)
    n_rel = len(id2relation)
    #supernode_id = len(id2concept)
    n_node = cids.shape[0]
    #adj = np.zeros((n_rel, n_node, n_node), dtype=np.uint8)
    #print(n_node)
    row = np.zeros(((n_node**2)*2*n_rel,))
    col = np.zeros(((n_node**2)*2*n_rel,))
    data = np.zeros(((n_node**2)*2*n_rel,))
    data = data.astype('int32')
    i=0
    for s in range(n_node):
        for t in range(n_node):
            s_c, t_c = cids[s], cids[t]
            if cpnet.has_edge(s_c, t_c) and qmask[s] and amask[t]:
                for e_attr in cpnet[s_c][t_c].values():
                    if e_attr['rel'] >= 0 and e_attr['rel'] < 2*n_rel:
                        #adj[e_attr['rel']][s][t] = 1
                        row[i] = s
                        col[i] = t
                        data[i] = e_attr['rel']
                        ...
                        # row[i] = t
                        # col[i] = s
                        # data[i] = e_attr['rel'] + 17
                        # i = i+1

            # else:
            #     is_q, is_a = qmask[s], amask[t]
            #     if is_q and is_a:
            #         row[i] = s
            #         col[i] = t
            #         data[i] = -1
    ...
    # for t in range(n_node+1):
    #     #t_c = cids[t]
    #     adj[n_rel-1][t][n_node] = 1
    adj = coo_matrix((data[:i], (row[:i], col[:i])), shape=(n_node, n_node))
    #node_ids.append(supernode_id)
    #cids = np.array(node_ids, dtype=np.int32)
    return adj, cids
...
def concepts_to_adj_matrices_2hop_all_pair(data):
    ...
    qmask = arange < len(qc_ids)
    amask = (arange >= len(qc_ids)) & (arange < (len(qc_ids) + len(ac_ids)))
    adj, concepts = concepts2adj(schema_graph, qmask, amask)
    #qmask = np.append(qmask, True)
    #amask = np.append(amask, True)
    return adj, concepts, qmask, amask
