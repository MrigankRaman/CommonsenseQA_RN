...
def train(args):
    ...
    dataset = LMRelationNetDataLoader(ablation = args.ablation, train_statement_path = args.train_statements, train_rpath_jsonl = args.train_rel_paths,
                                      dev_statement_path = args.dev_statements, dev_rpath_jsonl = args.dev_rel_paths,
                                      test_statement_path = args.test_statements, test_rpath_jsonl = args.test_rel_paths,
    ...
    model = LMRelationNet(model_name=args.encoder, non_adj_dim = 768, concept_num=concept_num, concept_dim=relation_dim,
...
def eval(args):
    ...
    model.eval()
    model.decoder.ablation = args.ablation
    ...
    #print(f'| dataset: {args.dataset} | num_dev: {dataset.dev_size()} | num_test: {dataset.test_size()} | save_dir: {args.save_dir} |')
    #dev_acc = evaluate_accuracy(dataset.dev(), model)
    dev_acc = 0
