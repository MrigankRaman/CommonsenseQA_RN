...
from modeling.modeling_gn_cpt_evidence_global_emb import *
...
def evaluate_accuracy(graph_loader, eval_set, model, device):
    ...
    with torch.no_grad():
        for graph, data in zip(graph_loader, eval_set):
            ...
            rel_ids = rel_ids.type(torch.LongTensor)
...
def train(args):
    ...
    GraphDataset = GraphDataLoader(args.train_adj, args.dev_adj, args.test_adj, args.mini_batch_size, args.eval_batch_size)
    ...
    for epoch_id in range(args.n_epochs):
        ...
        start_time = time.time()
        #i=0
        if epoch_id > args.unfreeze_epoch and epoch_id - best_dev_epoch >= 4:
           break
