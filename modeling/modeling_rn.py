...
from preprocess_tensors import *

class RelationNet(nn.Module):

    def __init__(self, non_adj_dim, concept_num, concept_dim, relation_num, relation_dim, sent_dim, concept_in_dim,
        ...
        #print(concept_in_dim, concept_dim)
       ...
       # self.non_adj_emb = CustomizedEmbedding(concept_num=concept_num, concept_out_dim=concept_dim,
       #                                        use_contextualized=use_contextualized, concept_in_dim=concept_in_dim,
       #                                        pretrained_concept_emb=pretrained_concept_emb, freeze_ent_emb=freeze_ent_emb,
       #                                        scale=emb_scale)

        encoder_dim = {'no_qa': relation_dim, 'no_2hop_qa': relation_dim, 'no_rel': concept_dim * 2}.get(self.ablation[1], concept_dim * 2 + relation_dim)
        if self.ablation[1] in ('encode_qas',):
        ...
        self.cpt_transform = nn.Linear(non_adj_dim, encoder_dim)               # transform gpt vectors to correct dimension

        if self.init_range > 0:
            self.apply(self._init_weights)

        if pretrained_relation_emb is not None and ablation[1] not in ('randomrel',):
    ...
    def forward(self, sent_vecs, qa_ids, rel_ids, num_tuples, non_adj_tensors, non_adj_num_tuples, emb_data=None):
        """
        ...
        non_adj_tensors: tensor of shape (batch_size, non_adj_max_tuple_num, non_adj_dim)
        non_adj_num_tuples: tensor of shape (batch_size,)
        (emb_data: tensor of shape (batch_size, max_cpt_num, emb_dim))
        """

        bs, sl, _ = qa_ids.size()
        _, sl_1, non_adj_dim = non_adj_tensors.size()             # get the size of gpt vectors
        mask = torch.arange(sl, device=qa_ids.device) >= num_tuples.unsqueeze(1)
        if self.ablation[1] in ('no_1hop', 'no_2hop', 'no_2hop_qa'):
            n_1hop_rel = int(np.sqrt(self.relation_num))
            assert n_1hop_rel * (n_1hop_rel + 1) == self.relation_num
            valid_mask = rel_ids > n_1hop_rel if self.ablation[1] == 'no_1hop' else rel_ids <= n_1hop_rel
            mask = mask | ~valid_mask
        mask[mask.all(1), 0] = 0  # a temporary solution for instances that have no qar-pairs
        mask2 = torch.arange(sl_1, device=qa_ids.device) >= non_adj_num_tuples.unsqueeze(1)
        if self.ablation[1] == 'no_2hop':
            mask2[mask2.all(1), 0] = 0      # create mask for the gpt_vectors
        final_mask = torch.cat((mask, mask2),-1)                                                  # concat the original mask and the non_adj_mask to get final_mask
        #print(final_mask.shape)
        qa_emb = self.concept_emb(qa_ids.view(bs, -1), emb_data).view(bs, sl, -1)
        rel_embed = self.rel_emb(rel_ids)
        #print(qa_emb.shape)
        if self.ablation[1] not in ('no_factor_mul',):
            n_1hop_rel = int(np.sqrt(self.relation_num))
            assert n_1hop_rel * (n_1hop_rel + 1) == self.relation_num
            #print(sl, self.relation_num, n_1hop_rel)
        ...
        if self.ablation[1] in ('no_qa', 'no_rel', 'no_2hop_qa'):
            concat = rel_embed if self.ablation[1] in ('no_qa', 'no_2hop_qa') else qa_emb
        else:
        ...
        non_adj_vecs = self.cpt_transform(non_adj_tensors)       # transfor the evidence vectors-
        non_adj_vecs = self.mlp(non_adj_vecs)                    # to same dimension-
        non_adj_vecs = self.activation(non_adj_vecs)             # as original vectors

        concated_vecs = torch.cat((qars_vecs, non_adj_vecs),1)         # concat the original and the generated vectors for attn pooling
        #print(concated_vecs.shape)
        if self.ablation[0] in ('multihead_pool', 'att_pool'):
            pooled_vecs, att_scores = self.attention(sent_vecs, concated_vecs, final_mask)        # attn pool the concatenated vecs
            # pooled_vecs_non_adj, att_scores_non_adj = self.attention(sent_vecs, non_adj_vecs)
            # pooled_vecs = (pooled_vecs + pooled_vecs_non_adj)*0.5
            #print(pooled_vecs.shape)
        else:
            qars_vecs = qars_vecs.masked_fill(mask.unsqueeze(2).expand_as(qars_vecs), 0)
            non_adj_vecs = non_adj_vecs.masked_fill(mask2.unsqueeze(2).expand_as(non_adj_vecs), 0)     # make the non zero padded values 0
            #print(mask2.unsqueeze(2).expand_as(non_adj_vecs).shape)
            pooled_vecs = (qars_vecs.sum(1) + non_adj_vecs.sum(1)) / ((~mask).float().sum(1).unsqueeze(1).float().to(qars_vecs.device) + ((non_adj_num_tuples).view(bs,1)).float().to(qars_vecs.device))   # meanpool all the evidence and the original vectors
            att_scores = None

        if self.ablation[1] == 'no_kg':
            pooled_vecs[:] = 0
...
class LMRelationNet(nn.Module):
    def __init__(self, non_adj_dim, model_name,
        ...
        self.decoder = RelationNet(non_adj_dim, concept_num, concept_dim, relation_num, relation_dim, self.encoder.sent_dim, concept_in_dim,
    ...
    def forward(self, *inputs, layer_id=-1):
        ...
        if self.use_contextualized:
            *lm_inputs, qa_ids, rel_ids, num_tuples, non_adj_tensors, non_adj_num_tuples, emb_data = inputs
        else:
            *lm_inputs, qa_ids, rel_ids, num_tuples, non_adj_tensors, non_adj_num_tuples = inputs              # take the tensors for evidence as input
            emb_data = None
        sent_vecs, all_hidden_states = self.encoder(*lm_inputs, layer_id=layer_id)
        logits, attn = self.decoder(sent_vecs=sent_vecs, qa_ids=qa_ids, rel_ids=rel_ids, num_tuples=num_tuples, non_adj_tensors=non_adj_tensors, non_adj_num_tuples=non_adj_num_tuples, emb_data=emb_data)  # cxy-style param passing
...
class LMRelationNetDataLoader(object):
    ...
    def __init__(self, ablation, train_statement_path, train_rpath_jsonl,
                 dev_statement_path, dev_rpath_jsonl,
                 test_statement_path, test_rpath_jsonl,
                 batch_size, eval_batch_size, device, model_name,
                 max_tuple_num=200, max_seq_length=128, max_non_adj_tuple_num=100,
        ...
        self.dev_data += load_2hop_relational_paths(dev_rpath_jsonl, dev_adj_path,
                                                    ...
                                                    max_tuple_num=max_tuple_num, num_choice=num_choice, node_feature_type=node_feature_type)
        if ablation[1] == 'no_2hop':
            evidence_features_train, evidence_num_tuples_train = create_tensors('cpt_pairs_train.gen.jsonl.gpt.layer-1.pt', max_non_adj_tuple_num , 'train_evidences_1.pt')  # preprocess the evidence tensors
            evidence_features_dev, evidence_num_tuples_dev = create_tensors('cpt_pairs_dev.gen.jsonl.gpt.layer-1.pt', max_non_adj_tuple_num , 'dev_evidences_1.pt')
        else:
            evidence_features_train, evidence_num_tuples_train = create_tensors('cpt_pairs_2hop_train.gen.jsonl.gpt.layer-1.pt', max_non_adj_tuple_num , 'train_evidences_1.pt')  # preprocess the evidence tensors
            evidence_features_dev, evidence_num_tuples_dev = create_tensors('cpt_pairs_2hop_dev.gen.jsonl.gpt.layer-1.pt', max_non_adj_tuple_num , 'dev_evidences_1.pt')

        self.train_data += [evidence_features_train, evidence_num_tuples_train]                     # add evidence tensors to the data
        self.dev_data += [evidence_features_dev, evidence_num_tuples_dev]
        ...
        if test_statement_path is not None:
            ...
            if ablation[1] == 'no_2hop':
                evidence_features_test, evidence_num_tuples_test = create_tensors('cpt_pairs_test.gen.jsonl.gpt.layer-1.pt', max_non_adj_tuple_num , 'test_evidences_1.pt')
            else:
                evidence_features_test, evidence_num_tuples_test = create_tensors('cpt_pairs_2hop_test.gen.jsonl.gpt.layer-1.pt', max_non_adj_tuple_num , 'test_evidences_1.pt')
            self.test_data += [evidence_features_test, evidence_num_tuples_test]
