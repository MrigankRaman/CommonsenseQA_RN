...
def open_graph(graph_path, tensor_path):
    d = torch.load(tensor_path)
    #gen_emb = d['all_evidence_vecs']
    input = torch.empty(34, 768)
    yu = torch.zeros_like(input)
    gen_emb = torch.cat([yu, d['all_evidence_vecs']], dim=0)
    ...
    for graph in tqdm(graph_list):
        row = graph[0].row
        col = graph[0].col
        edge_attr = torch.cat([torch.tensor(graph[0].data).view(torch.tensor(graph[0].data).shape[0],1), gen_emb[graph[0].data]], dim=1)
...
class GraphDataLoader(object):
    def __init__(self, train_adj_path, train_tensor_path, dev_adj_path, dev_tensor_path, batch_size, eval_batch_size):
        self.batch_size = 5*batch_size
        self.eval_batch_size = 5*eval_batch_size
        self.train_graph = open_graph(train_adj_path, train_tensor_path)
        self.dev_graph = open_graph(dev_adj_path, dev_tensor_path)
        #self.test_graph = open_graph(test_adj_path, train_tensor_path)
...
class GraphNet(nn.Module):
    def __init__(self, concept_num, concept_dim, relation_num, relation_dim, sent_dim, concept_in_dim,
                 ...
                 pretrained_concept_emb=None, pretrained_relation_emb=None, freeze_ent_emb=True, num_layers = 1,
        ...
        #self. gen_emb = nn.Embedding()
        self.gen_transform = nn.Linear(768, relation_dim)
        ...
        # self.generated_emb = CustomizedEmbedding(concept_num=evidence_num, concept_out_dim=concept_dim,
        #                                        use_contextualized=False, concept_in_dim=768,
        #                                        pretrained_concept_emb=generated_emb, freeze_ent_emb=True,
        #                                        scale=emb_scale)
        encoder_dim =  concept_dim * 2 + relation_dim
        edge_in_dim = encoder_dim + 128
        self.transform = nn.Linear(sent_dim, 128)
        self.num_layers = num_layers
        self.EdgeModel = EdgeModel(edge_in_dim, 128, 128)
        self.NodeModel = NodeModel(concept_dim, 128, 128, 128, 128)
        self.GlobalModel = None
        self.Network = MetaLayer(self.EdgeModel, self.NodeModel, self.GlobalModel)
        if self.num_layers==2:
            self.EdgeModel_2 = EdgeModel(128*4 , 128, 128)
            self.NodeModel_2 = NodeModel(128, 128, 128, 128, 128)
            self.Network_2 =  MetaLayer(self.EdgeModel, self.NodeModel, None)
    ...
    def forward(self, edge_index, c_ids, u, batch, num_tuples, rel_ids, emb_data = None):
        ...
        rel_edges = rel_ids[:,0].type(torch.LongTensor)
        rel_edges = rel_edges.to(device=rel_ids.device)
        edge_mask = rel_ids[:,0]<34
        edge_mask = edge_mask.to(device=rel_ids.device)
        rel_edges = rel_edges*edge_mask
        edges = rel_ids[:,1:]
        edges = edges.to(device=rel_ids.device)
        edge_attr = self.gen_transform(edges)
        edge_attr = edge_attr.masked_fill(edge_mask.unsqueeze(1).expand_as(edge_attr), 0)
        #print(edge_mask.unsqueeze(1).shape)
        edge_attr = edge_attr + edge_mask.unsqueeze(1)*self.rel_emb(rel_edges)
        u = self.transform(u)
        x, edge_attr, u_1 = self.Network(node_attr, edge_index, edge_attr, u,batch)
        if self.num_layers==2:
            x, edge_attr, u_1 = self.Network_2(x, edge_index, edge_attr, u,batch)
...
class Decoder(nn.Module):
    ...
    def __init__(self, concept_num, concept_dim, relation_num, relation_dim, sent_dim, concept_in_dim,
        ...
        self.GraphNet = GraphNet(concept_num, concept_dim, relation_num, relation_dim, sent_dim, concept_in_dim,
                     hidden_size, num_hidden_layers, num_attention_heads, fc_size, num_fc_layers, dropout, pretrained_concept_emb=pretrained_concept_emb, pretrained_relation_emb=pretrained_relation_emb, freeze_ent_emb=freeze_ent_emb, num_layers = num_layers,
    ...
    def forward(self, sent_vecs, edge_index, c_ids, u, batch, num_tuples, rel_ids, max_tuple_num = 200, emb_data = None):
        ...
        mask[mask.all(1), 0] = 0
        node_vecs, edge_vecs, global_vecs = self.GraphNet(edge_index, c_ids, sent_vecs, batch, num_tuples, rel_ids, emb_data)
...
class LMGraphNet(nn.Module):
    ...
    def __init__(self, model_name,
                 ...
                 pretrained_relation_emb=None, freeze_ent_emb=True, num_layers = 1, init_range=0, ablation=None,
        ...
        self.decoder = Decoder(concept_num, concept_dim, relation_num, relation_dim, self.encoder.sent_dim, concept_in_dim,
                                   ...
                                   freeze_ent_emb=freeze_ent_emb, num_layers = num_layers, init_range=init_range, ablation=ablation,
